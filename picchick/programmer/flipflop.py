...
class FlipflopProgrammer(SerialProgrammer):
    ...
    def add_args(parser):
        ...
        super(FlipflopProgrammer, FlipflopProgrammer).add_args(parser)
        # Add additional flags we support
    ...
    def read(self, address, length):
        ...
        self._conn.write(READ + INTBYTES(address) + INTBYTES(length*2))
        self._conn.read(5)
        resp = bytearray(self._conn.read(length*2))
        resp_crc = self._conn.read(2)
        crc_check = crc16(resp + resp_crc)
        if crc_check != 0:
            print('failed')
            return None
        print('success')
    ...
    def write(self, address, data):
        ...
        crc_cal = crc16(data)
        cmd = WRITE + INTBYTES(address) + INTBYTES(len(data)) + data + crc_cal.to_bytes(2, 'big')
        self._conn.write(cmd)
        self._conn.read(len(cmd))
...
# This code currently uses a lookup table to compute a crc byte by byte.
# Taken from a comment on this gist:
# https://gist.github.com/oysstu/68072c44c02879a2abf94ef350d1c7c6#gistcomment-3943460

def crc16(data: bytes):
    '''
    CRC-16 (CCITT-FALSE) implemented with a precomputed lookup table
    '''
    table = [ 
