...
:link-mchip-dfu-repo: https://packs.download.microchip.com/
...
developed using the latest version of python (currently 3.13). +
...
Microchip Device Family Packs _(optional)_::
Microchip provides device configuration files via Device Family Packs (DFPs).
Picchick comes bundled with PIC10, PIC12, PIC16, and PIC18 device files.
However, in certain circumstances these could be outdated or not contain a
device. In these cases, you should download the needed DFP from the
{link-mchip-dfu-repo}[Microchip Packs Reposity] and install it in
`~/.mchp_packs` or tell picchick where it is located with the `MCHP_PACKS`
...
== Usage
`picchick` is meant to be used as a general programming utility.
...
[cols='1,3']
|===
|Target |Compiler Flag Equivalent
...
<...>
programmer options:
  -P, --port port       programmer serial port
  -B, --baud baud       serial connection baudrate
  --bulk-erase          use bulk erase when erasing device
...
words which is based on the word_size of the device. Both arguments can be
prefixed with `0x` to specify hexadecimal numbers.
...
$ picchick -c <programmer> -d <mcu> -r 0x400

# Read a block of 1024 words from 0x400
$ picchick -c <programmer> -d <mcu> -r 0x400 1024
----
====
...
----
 ...
 # Write 0xbeef to 0x400
 $ picchick -c <programmer> -d <mcu> -w 0x400 0xbeef

 # Write 42 to 0x400
 $ picchick -c <programmer> -d <mcu> -w 0x400 42
----
====
...
programmer) the block containing the address will be erased. Some programmers
(picstick) have a `--bulk-erase` flag that will bulk erase the device instead
of block erasing.

.Erasing a device
====
 $ picchick -c <programmer> -d <mcu> -e <addr> [--bulk-erase]
====
